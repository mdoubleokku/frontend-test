      ...
      <v-col>
        ...
        <div>
          <v-row>
            ...
            <b class="nameColor">{{item.name}} </b>
            <span>posted on {{ item.created_at | formatDate }}</span>
            <hr>
            </v-col>
          </v-row>
          <v-row class="ma-0">
...
export default {
  ...
  computed: {
    forumList () {
      return this.postsList.reduce((returnArray, item) => {
      let match = this.authorsList.find(el => el.id == item.author_id)
      if (match) {
        returnArray.push({ ...item, name: match.name, avatar_url: match.avatar_url})
      }
        return returnArray
      }, [])
    }
